...
export default function ClientsPage() {
  ...
  // Two-level state for both inputs: raw (what user types) → debounced (used for API)
  ...
  // Debounce search input → search (used in API call)
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  // Debounce username input → username (used in API call) + persist + auto-switch tab
  useEffect(() => {
    const t = setTimeout(() => {
      const trimmed = usernameRaw.trim();
      setUsername(trimmed);
      setPage(1);
      // Auto-switch to "My Clients" when a username is typed
  ...
  // Reset to page 1 when filter tab changes
  useEffect(() => {
    setPage(1);
  }, [filterType]);

  // Fetch whenever page / search / filterType / username / reloadToken change
  useEffect(() => {
    let cancelled = false;
  ...
  const downloadExcel = async () => {
    setDownloading(true);
    try {
      // Fetch all pages for the current filter
    ...
    <div className="min-h-screen bg-slate-50 pb-16">
      ...
      <main className="mx-auto mt-6 flex max-w-6xl flex-col gap-6 px-4">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          {/* Username filter — always visible, auto-applies on typing */}
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-emerald-300 hover:shadow-md">
      ...
      <div className="flex items-start justify-between gap-3 p-5 pb-4">
        ...
        {client.client_type && (
          <span
            className={`shrink-0 rounded-full border px-2.5 py-0.5 text-xs font-medium ${typeClass}`}
          >
            {client.client_type}
          </span>
        )}
      ...
      <div className="grid grid-cols-2 gap-3 p-5 pt-4 text-sm">
        ...
        {client.medad_status && (
          <DetailRow
            ...
            label="Status"
            value={client.medad_status as string}
          />
        )}
      </div>
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="h-52 animate-pulse rounded-2xl border border-slate-100 bg-slate-100"
        />
      ))}
    </div>
  );
